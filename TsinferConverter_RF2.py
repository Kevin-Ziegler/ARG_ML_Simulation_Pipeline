...
def getTotalRFDistance(fileNameNewick, lstWeights, L , MR):
	flagTsinfer = "1"
	s = subprocess.check_output(["Rscript", "/pool/Kevin81/LemmonLab/AncestralRecombinationGraphGeneTree/Rescale_RF_File.r", fileNameNewick, flagTsinfer, L, MR])
	s = s.decode('utf-8')
	#print(s)
...
for fileName in lst_FileNames:
	...
	if ".phy_m" in fileName:
		...
		ftempNewick.close()
		totalDist = getTotalRFDistance(estimated + "_Newick", lstWeight, length, mutationRate)
